...
@Component({
  ...
  templateUrl: './ressources-form.page.html',
  styleUrls: ['./ressources-form.page.scss'],
})
export class RessourcesFormPage implements OnInit {
              ...
              buttons: [
                {
                  ...
                  handler: () => {
                    this.navCtrl.navigateRoot(['/enseignant']).then(() => {
                      this.navCtrl.navigateRoot(['/enseignant/ressources'], {
  ...
  async recordAudio() {
    ...
    try {
      let filelabel = filePath.split('/').pop();
      const directory = await this.File.resolveDirectoryUrl(
        filePath.replace(filelabel, '')
      );
